...
The core discriminator is structural and relational: **does the relevant result depend on changing the target's contestability?** The relevant result is the practical ordering in the specified field/channel affected by the mechanism. An antecedent process counts as contest-mediated only insofar as it engages the target's contribution within that ordering; deciding whether to restrict participation is not itself a substitute for the contest whose shortcut is at issue.
...
- These studies establish position-dependent ambient conditions and disclosure responses, not the historical sequence required to show that a particular shortcut became sedimented. Their role is to test whether an inherited norm can itself structure contestability without a discrete current ban.
